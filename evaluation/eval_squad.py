#!/usr/bin/env python3
# Evaluate SQUAD predictions with lenient matching
...
def normalize_answer(s: str) -> str:
    ...
    return white_space_fix(remove_articles_and_common_stopwords(remove_punc(lower(s))))


# Return int value if text is a number or number word
...
def _token_subset_match(a: str, b: str) -> bool:
    ...
    if len(small) >= 3 and abs(len(toks_a) - len(toks_b)) > 5:
        return False
