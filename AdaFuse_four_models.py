...
# Worker function for multi-GPU ensemble decoding with 4 models
def ensemble_decoding_worker(gpu_id, start_idx, end_idx, args, prompt_complex):
    ...
    print(f"GPU {gpu_id}: Loading model4...")
    ...
    tokenizer4 = AutoTokenizer.from_pretrained(args.model_path4, trust_remote_code=True)
    ...
    generation_config4 = GenerationConfig(
        num_beams=1,
        num_return_sequences=1,
        do_sample=False,
        ...
        for idx, prompt in enumerate(questions):
            ...
            for i in range(args.max_span_rounds):
                ...
                candidates3 = generate_candidate_spans(model3, tokenizer3, current_prefix,
                                                       generation_config3,
                                                       args.theta_delta, args.max_words)
                candidates4 = generate_candidate_spans(model4, tokenizer4, current_prefix,
                                                       generation_config4,
                                                       args.theta_delta, args.max_words)
                ...
                with open(log_file, "a", encoding="utf-8") as logf:
                    ...
                    for model_tag, cand in [("Model1", candidates1), ("Model2", candidates2), 
                                            ("Model3", candidates3), ("Model4", candidates4)]:
                    ...
                    ppl_scores = []
                    nan_count = 0
                    for span_txt, (span1, span2, span3, span4) in cands.items():      
                        ...
                        if span4 is not None:
                            ...
                            tok_ids4 = tokenizer4(span_txt, add_special_tokens=False)["input_ids"]
                        ...
                        ppl4 = compute_span_perplexity(model4, tokenizer4, current_prefix, tok_ids4)
                        ...
                        if not np.isnan(ppl4):
                            valid_ppls.append(ppl4)
                        ...
                        logf.write(f'  PPL(model4): {ppl4:.3f}\n')
                    ...
                    if not ppl_scores:
                        ...
                        logf.write(f"  Model4 generated {debug_info['model4_count']} candidate(s): {debug_info['model4_spans']}\n\n")
                        logf.write(f"  Total unique spans collected: {len(cands)}\n")
                        logf.write(f"  Spans with all PPLs NaN (skipped): {nan_count}\n\n")
                        
                        if len(cands) == 0:
                            logf.write(f"  Root Cause: 🔴 All four models failed to generate any candidate spans.\n")
                            ...
                            logf.write(f"  Root Cause: 🔴 All candidate spans have NaN PPL values from all four models.\n")
                        ...
                        print(f"   M1: {debug_info['model1_count']}, M2: {debug_info['model2_count']}, M3: {debug_info['model3_count']}, M4: {debug_info['model4_count']} candidates")
                        print(f"   Total unique spans: {len(cands)}, NaN PPL count: {nan_count}")
                        if len(cands) == 0:
                            print(f"   Root cause: All four models failed to generate any candidate spans")
                        elif nan_count == len(cands):
                            print(f"   Root cause: All {len(cands)} span(s) have NaN PPL from all four models")
                ...
                if (tokenizer1.eos_token and tokenizer1.eos_token in selected_span) or \
                   ...
                   (tokenizer3.eos_token and tokenizer3.eos_token in selected_span) or \
                   (tokenizer4.eos_token and tokenizer4.eos_token in selected_span):
                    break
...
def main():
    ...
    arg_parse.add_argument("--model_path4", type=str, required=True)
    ...
    print('Multi-GPU ensemble decoding with 4 models completed!')
